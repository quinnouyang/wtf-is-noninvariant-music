...
def penalty_ls(x1, y1, x2, y2, t=1, reg=0.1):
    phi = np.diag([1, t])
...
if __name__ == "__main__":
    ...
    plt.plot(
        plot_x, plot_y_ls, lw=2, label=r"$\mathbb{D}_{\text{dist}}((1, 0), \Phi, e)$"
    )
    plt.plot(
        plot_x,
        plot_y_ls_reg,
        ls="--",
        lw=2,
        label=r"$\mathbb{D}_{\text{dist}}$ (heavy regularization)",
    )
    plt.plot(
        plot_x, plot_y_1, ".", lw=2, label=r"$\mathbb{D}_{\text{lin}}((1, 0), \Phi, e)$"
    )
    plt.ylim(-1, 12)
    plt.xlabel(
